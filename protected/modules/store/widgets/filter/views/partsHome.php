...
if(!empty($active))
{
	echo CHtml::openTag('div', array('class'=>'rounded'));
		...
		echo CHtml::link(Yii::t('StoreModule.core','Сбросить фильтр'), $this->getOwner()->model->viewUrl, array('class'=>'cancel_filter'));
	echo CHtml::closeTag('div');
}
?>
...
if(!empty($manufacturers['filters']) || !empty($attributes))
	...
	foreach($attributes as $attrData)
	{
        ...
        if($attrData['type'] == StoreAttribute::TYPE_NUMBER)
        {
            $minQueryKey = "min_".$attrData['queryKey'];
            $maxQueryKey = "max_".$attrData['queryKey'];
        ?>
            <?php echo CHtml::form() ?>
            	от <?php echo CHtml::textField($minQueryKey, (isset($_GET[$minQueryKey])) ? (int)$_GET[$minQueryKey]:null ) ?>
            	до <?php echo CHtml::textField($maxQueryKey, (isset($_GET[$maxQueryKey])) ? (int)$_GET[$maxQueryKey]:null ) ?> 
            	<button class="small_silver_button" type="submit">OK</button>
            <?php echo CHtml::endForm() ?>          
        <?
        }
