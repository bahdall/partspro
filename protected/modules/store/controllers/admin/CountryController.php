 ...
 */
class CountryController extends SAdminController
{
	...
	{
		$model = new StoreCountry('search');


		if (!empty($_GET['StoreCountry']))
			$model->attributes = $_GET['StoreCountry'];
	...
	{
		if ($new === true)
			$model = new StoreCountry;
		else
		{
			$model = StoreCountry::model()
				->language($_GET)
				->findByPk($_GET['id']);
		}
        ...
        $form = new STabbedForm('application.modules.store.views.admin.country.countryForm', $model);
		
		
		if (Yii::app()->request->isPostRequest)
		{
			$model->attributes = $_POST['StoreCountry'];
	...
	public function actionDelete($id = array())
	{
		if (Yii::app()->request->isPostRequest)
		{
			$model = StoreCountry::model()->findAllByPk($_REQUEST['id']);
